...
@Component({
  selector: 'page-district-map',
  templateUrl: 'district-map.html'
})
export class DistrictMapPage {
  ...
  public districtCenter = [
    [41.380833, 2.173056], // Ciutat Vella
    [41.390364514520044,2.1643066406250004], // L'Eixample
    [41.372493252775456,2.140917778015137], // Sants-Montjuïc
    [41.384601166653376,2.1256828308105473], // Les Corts
    [41.398611, 2.131111], // Sarrià-St. Gervasi
    [41.406667, 2.158056], // Gràcia
    [41.414635801450366,2.1702289581298833], // Horta-Guinardó
    [41.43468335377089,2.1731901168823247], // Nou Barris
    [41.43188414766482,2.1897983551025395], // Sant Andreu
    [41.40530176391292,2.1987676620483403] // Sant Martí
  ];
  ...
  ionViewWillEnter() {
    this.platform.ready().then(() => {
      ...
      this.analytics.trackPage('district/'+this.district.toString()+'/map');
  ...
  getTemplateData(reload?: boolean, newCategories?: boolean) {
    ...
    this.poisData.getPoisList(this.count, 0, undefined, undefined, this.district, categoryVal).then((data) => {
  ...
  loadMap() {
    ...
    // Set default view to district's center
    this.map.setView(this.districtCenter[(this.district-1)], 15);
  ...
  openDesktopMoreCategories(ev) {
    let popover = this.popoverCtrl.create(DistrictMapPagePopover, {
  ...
  changeView() {
    this.navCtrl.setRoot(DistrictPage, {
...
export class DistrictMapPagePopover {
